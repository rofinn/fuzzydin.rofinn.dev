...
export default function Table(props: Props) {
  ...
  let cells = [
    <div key="weight-header" className="text-md font-bold p-2">
      Weight (kg)
    </div>,
    <div key="height-header" className="text-md font-bold p-2">
      Height (cm)
    </div>,
    <div key="skier-header" className="text-md font-bold p-2">
      Skier Code
    </div>,
    <div key="lenght-header" className="text-md font-bold p-2 col-span-8">
      Boot Length (mm)
    </div>,
  ...
  for (let i = 0; i < BOOT_LENGTHS.length; i++) {
    ...
    cells.push(
      <div key={keyString(-1, i)} className={className}>
        {intervalString(len)}
      </div>,
    );
  }
  ...
  for (let i = 0; i < SKIER_WEIGHTS.length; i++) {
    ...
    cells.push(
      <div
        ...
        className="text-xs font-thin tracking-tighter"
      >
        {i > offset ? intervalString(SKIER_HEIGHTS[i - offset]) : ""}
      </div>,
    );

    let widx = props.results.skier.codes.findIndex(
    ...
    cells.push(
      <div key={keyString(i, -1)} className={wClassName}>
        {weight.code}
      </div>,
    );
    ...
    for (let j = 0; j < BOOT_LENGTHS.length; j++) {
      ...
      if (widx >= 0 && lidx >= 0) {
        let weight =
          props.results.skier.weights[widx] * props.results.boot.weights[lidx];
        if (weight <= 0.25) {
          className =
            "rounded text-md font-light tracking-tight bg-gray-300 text-white";
        } else if (weight <= 0.5) {
          className =
            "rounded text-md font-medium tracking-tight bg-gray-400 text-white";
        } else if (weight <= 0.75) {
          className =
            "rounded text-md font-medium tracking-tight bg-gray-500 text-white";
        } else {
          className =
            "rounded text-md font-medium tracking-tight bg-gray-700 text-white";
        }
      }
      cells.push(
        <div key={keyString(i, j)} className={className}>
          {len.code in DIN[weight.code]
            ? DIN[weight.code][len.code].toString()
            : ""}
        </div>,
      );
    }
